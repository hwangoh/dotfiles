...
\begin{enumerate}
    ...
    \item Use <c-w>N in a Vim terminal to switch to normal mode, which allows
...
maps:
\begin{lstlisting}
onoremap <silent> <expr> - v:count==0 ? ":<c-u>normal! ^vg_<cr>" : ":<c-u>normal! V" . (v:count) . "jk<cr>"
vnoremap <silent> <expr> - v:count==0 ? ":<c-u>normal! ^vg_<cr>" : ":<c-u>normal! V" . (v:count) . "jk<cr>"
...
I have extended the plugin with SendTextToTerminalRight(), SendVariableRight()
and SendMarkedSectionRight() functions (and for the other three directions as
...
" sendtowindow for IPython in Vim Terminal: clear, reset, run code, run variable, run marked section
    noremap ,sL :call SendTextToTerminalRight('clear')<CR>
    noremap ,sD :call SendTextToTerminalRight('%reset -f')<CR>
    nmap ,sR :w!<CR>:call SendTextToTerminalRight('run <c-r>%')<CR>
...
" IPython, Clear, Reset, Run Variable, Run Marked Section and Run Code
    nnoremap ,P :SlimuxShellRun ipython<CR>
    nnoremap ,L :SlimuxShellRun clear<CR>
    nnoremap ,D :SlimuxShellRun %reset -f<CR>
    nmap ,V mqviw,-`qdmq
    nmap ,M mq'xV'z,-`qdmq
    nnoremap ,R :SlimuxShellRun run <c-r>%<CR>
\end{lstlisting}
where SlimexShellRun awaits a command to send to shell and SlimuxShellLast runs
the last shell command. For the last command that runs the Python code, we
leverage the fact that `\tlangle c-r\trangle' in command mode will prime pasting
from the register and \% in the register stores the file name. The same can't be
done with sendtowindow as that plugin does not utilize the command line;
only items present in the text can be sent to the terminal window.
