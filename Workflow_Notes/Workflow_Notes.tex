...
\begin{enumerate}
    \item Move ~/.vimrc into ~/.vim.
    \item When vim boots, it's still going to look for .vimrc in the home directory. To ensure that it looks for vimrc
        in the ~/.vim directory, we can create a symbolic link to that file using `ln -s ~/.vim/vimrc ~/.vimrc'.
    \item Make ~/.vim into a git repository.
\end{enumerate}
...
imported into a new machine. To fo so, use the following:
\begin{enumerate}
    \item cd ~
    \item git clone \tlangle git repo url\trangle ~/.vim
    \item ln -s ~/.vim/vimrc ~/.vimrc
    \item cd ~/.vim
...
\subsection{Vim-plug}
%---------------------------------------------------------------   
Whilst Pathogen is the simplest plugin manager, there are limitations when porting your setup to a new machine. For
example, suppose one of the plugins is no longer being maintained by the owner and suppose also that you've made your
own changes to the plugin. If you were to pull your configuration on a new machine, you will pull the latest version of
the plugin; that is your changes will not be ported. Further, on your own repository for your configuration, the
directories containing the plugins will be treated as repositories. Unless you manually install the plugins, there is
no way to preserve your changes onto Github.
...
left, right, above or below the Vim window.

Some notes on installation: notice that this repository only has one file called sendtosplit.vim which contains a
couple of functions. Unless you are using a plugin manager like vim-plug or put all these functions into your vimrc, we
can install this plugin in a slightly different way. All we need to do is create a directory ~/.vim/plugin and copy
sendtosplit.vim into that directory. However, unlike the method with pathogen detailed in Section \ref{SecPathogen},
your plugin won't be version controlled and so any updates added by the coder cannot be retrieved via git. More details
about this type of plugin management can be found in \cite{ryder2018attack}.\\

This plugin is simple and elegant in use. I use the following maps:\\
\begin{lstlisting}
